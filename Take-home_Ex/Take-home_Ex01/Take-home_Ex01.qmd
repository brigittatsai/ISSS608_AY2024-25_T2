...
ggplot(data = ship, aes(x = Profit_per_Voyage_USD)) + 
  ...
  theme_gray()
```
The histogram shows a normal distribution ranging from US$ -500,000 to US$ 1,000,000. The distribution of profit is symmetrical with the highest count at US$ 250,000. Normal distribution shows that it is uncommon to have a very high profit, and it is also uncommon to get extreme losses from ship journey. The x-axis shows there are presence of negative profit value, ranging from US$ 0 to US$ -500,000. Although the occurence is not as often, is is still a questionable event of why certain ships incur losses in the ship operations. The histogram roughly highlights financial variability in the ship and maritime industry. 
